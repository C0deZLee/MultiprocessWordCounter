...
void print_result(char *result_filename, char *runtime_filename, long runtime, char **file, int w){
    ...
    fp = fopen(result_filename, "w+");                                  // assign the file pointer

    for(int i=0; i<w; i++) search_in_list(file[i]);
    for(struct word_count *curr = HEAD;curr != NULL;curr = curr->next)  // print the result into result_filename
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5){
        printf("Expected 4 arguments, given %d", argc-1);              // no enough argv
        exit(1);
    }
    char *p;
    errno = 0;
    int conv = strtol(argv[4], &p, 10);
    if (errno != 0 || *p != '\0' || conv > INT_MAX) {
        printf("The 4th argument should be a number!\n");              // error on argv[4]
        exit(1);
    } else {
        TOTAL_PROCESS_NUM = conv;                                      // No error for argv[4]
    }
    ...
    print_result(argv[2], argv[3],runtime,tokenized_file,total_num_of_words);// print the result
